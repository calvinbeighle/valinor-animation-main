...
class ValinorAnimation {
    ...
    private animationState: AnimationState = 'vertical-center';
    private isAnimating: boolean = false;
    private animationId: number | null = null;
    private startTime: number = 0;
    private radius: number = 130; // Distance from center to hexagon vertices
    private centerX: number = 485; // Center X for the screen (positioned between computer and left formation) - moved 15px left total
    private centerY: number = 300; // Center Y for the screen
    ...
    private originalPositions: Array<{x: number, y: number}> = []; // Store original positions
    ...
    private isComputerAnimating: boolean = false;
    private computerAnimationId: number | null = null;
    private computerStartTime: number = 0;
    private computerContent: HTMLElement;
    private currentCodeLine: number = 0;
    private currentPhase: 'tool-call' | 'code' | 'question' | 'response' = 'tool-call';
    ...
    private initializeVerticalFormation(): void {
    ...
    private transitionToLeftPosition(): void {
    ...
    private transitionToRightPosition(): void {
    ...
    private createWebLines(): void {
        ...
        const originalHexagonPoints: Array<{x: number, y: number}> = [];
        for (let i = 0; i < 6; i++) {
    ...
    private generateContinuousWebPath(points: Array<{x: number, y: number}>, type: string = 'original'): void {
    ...
    private scaleComputerScreenDown(): void {
        const computerContainer = document.querySelector('.computer-container') as HTMLElement;
        if (computerContainer) {
    ...
    private resetComputerScreenScaling(): void {
        const computerContainer = document.querySelector('.computer-container') as HTMLElement;
        if (computerContainer) {
    ...
    private animateComputer = (currentTime: number): void => {
        if (!this.computerStartTime) this.computerStartTime = currentTime;
        ...
        let phase: 'question' | 'tool-call' | 'code' = 'question';
        let phaseProgress = 0;
    ...
    private updatePhaseProgress(phase: 'question' | 'tool-call' | 'code', progress: number, cycleIndex: number): void {
        const questionElement = this.computerContent.querySelector('.question-bubble') as HTMLElement;
        const toolCallElement = this.computerContent.querySelector('.tool-call-bubble') as HTMLElement;
        const codeContainer = this.computerContent.querySelector('.code-flow-container') as HTMLElement;
            ...
            if (codeContainer) {
                ...
                codeLines.forEach((line, index) => {
                    ...
                    if (index < linesToShow) {
                        ...
                        const typingContent = lineElement.querySelector('.typing-content') as HTMLElement;
    ...
    private updateComputerDisplay(): void {
    ...
    private createCycleDisplay(): void {
    ...
    private startTypingAnimation(lineElement: HTMLElement, lineIndex: number): void {
