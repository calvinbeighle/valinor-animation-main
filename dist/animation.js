class ValinorAnimation {
    constructor() {
        ...
        this.centerX = 500; // Center X (moved further right)
        this.centerY = 300; // Center Y (vertically centered with original linear positions)
        ...
        this.currentIconIndex = 0; // Track which icon is being processed
        this.iconsProcessed = 0; // Track how many icons have been processed
        ...
        this.animateGentleBlink = (currentTime) => {
            if (!this.auraStartTime)
                this.auraStartTime = currentTime;
            ...
            endpointPoints.forEach((endpoint) => {
                ...
                const gradient = this.webCtx.createRadialGradient(endpointX, endpointY, 0, endpointX, endpointY, glowRadius);
    ...
    generateContinuousWebPath(points) {
    ...
    drawGlowingNodes(hexagonPoints, progress) {
    ...
    updateComputerContent() {
        ...
        if (this.computerState === 'detecting') {
            ...
            const iconClone = currentIcon.element.cloneNode(true);
            ...
            // Create a clean copy of the hexagon icon without original positioning
            const iconClone = runningIcon.element.cloneNode(true);
            iconClone.style.position = 'relative';
            iconClone.style.left = '0';
            iconClone.style.top = '0';
            iconClone.style.transform = 'none';
